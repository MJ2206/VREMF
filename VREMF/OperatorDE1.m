function Offspring = OperatorDE1(Problem,Parent1,Parent2,Parent3,Parent4)
...
%--------------------------------------------------------------------------
    ...
    B = [min(Parent1.decs); max(Parent1.decs); min(Parent4); max(Parent4)];
    ...
    Offspring       = Parent1;
    Offspring(OS)   = Offspring(OS) + F*(Parent2(OS)-Parent3(OS));
    for i = 1:N
        if rand < 0.5
            ob = Parent1(randi(N),:);
            Offspring(i,UOS) = ob(1,UOS);
        else
            ob = B(randi(4),:);
            Offspring(i,UOS) = ob(1,UOS);
        end
